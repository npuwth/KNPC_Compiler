 ...
 */
int knpc::err::numOfErrors(void) { return num_of_errors; }
 ...
 */
void knpc::err::debug(const char *msg, ...) {
    using namespace std;
    va_list ap;
 ...
 * NOTE:
 *   do NOT call me directly. please use 'knpc_assert(...)' instead.
 */
void knpc::err::bad_assertion(const char *msg, const char *file, int line) {
...
NotMethodError::NotMethodError(Symbol *s) {
    knpc_assert(NULL != s && !s->isFunction());
...
NotVariableError::NotVariableError(Symbol *s) {
    knpc_assert(NULL != s && !s->isVariable());
