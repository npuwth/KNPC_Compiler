...
namespace IR
{
    ...
    struct Instr : Print
    {
        ...
        void map_BasicBlock(std::function<void(BasicBlock *&)> f)
        {
            map([](auto &x) {}, f, [](auto &x) {}, 0);
        }
        Instr *copy()
        {
            return map([](Reg &) {}, [](BasicBlock *&) {}, [](MemObject *&) {}, 1);
        }
        Instr *map(function<void(Reg &)> f1, function<void(BasicBlock *&)> f2,
                   function<void(MemObject *&)> f3, bool copy = 1);
        // copy this Instr, and map each Reg by f1, map each BasicBlock* by f2, map each
        // MemObject by f3
    };
    struct PhiInstr;
    struct BasicBlock : Print
    {
        // basic block
        string name;
    ...
    struct NormalFunc : Func
    {
        ...
        int max_reg_id = 0, max_BasicBlock_id = 0;
        // for id allocation
        ...
        BasicBlock *new_BasicBlock(string _name = "BasicBlock")
        {
            BasicBlock *BasicBlock = new BasicBlock(name + "::" + _name + to_string(++max_BasicBlock_id));
            BasicBlocks.emplace_back(BasicBlock);
            return BasicBlock;
        }
        void for_each(function<void(BasicBlock *)> f)
        {
            for (auto &BasicBlock : BasicBlocks)
                f(BasicBlock.get());
        }
        bool for_each_until(function<bool(BasicBlock *)> f)
        {
            for (auto &BasicBlock : BasicBlocks)
                if (f(BasicBlock.get()))
                    return 1;
    ...
    struct JumpInstr : ControlInstr
    {
        // PC = target
        BasicBlock *target;
        JumpInstr(BasicBlock *target) : target(target) {}
        void print(ostream &os) const override;
    };
    ...
    struct BranchInstr : ControlInstr
    {
        ...
        Reg cond;
        BasicBlock *target1, *target0;
        BranchInstr(Reg cond, BasicBlock *target1, BasicBlock *target0)
    ...
    struct PhiInstr : RegWriteInstr
    {
        ...
        vector<std::pair<Reg, BasicBlock *>> uses;
        PhiInstr(Reg d1) : RegWriteInstr(d1) {}
        void add_use(Reg r, BasicBlock *pos) { uses.emplace_back(r, pos); }
        void print(ostream &os) const override;
    };
